-- Firecrawl NUQ schema for local self-hosted (no pg_cron)
-- Based on firecrawl/apps/nuq-postgres/nuq.sql, with cron jobs removed

CREATE EXTENSION IF NOT EXISTS pgcrypto;
-- pg_cron not available in standard Homebrew PostgreSQL, skip it
-- Cron jobs are cleanup-only (delete old completed/failed jobs), not needed for local dev
