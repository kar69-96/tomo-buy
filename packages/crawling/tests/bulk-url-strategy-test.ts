/**
 * Bulk URL test for strategy-aware discovery pipeline.
 * Compares discoverWithStrategy (new) against the same URL set.
 *
 * Usage: set -a && source .env && set +a && npx tsx packages/crawling/tests/bulk-url-strategy-test.ts
 */

import { discoverWithStrategy } from "../src/discover.js";
import { classifyUrl, type DiscoveryStrategy } from "../src/url-classifier.js";
...
async function runTest(
  ...
  const strategy = classifyUrl(entry.url);
  const start = Date.now();
  try {
    const result: FullDiscoveryResult | null = await discoverWithStrategy(entry.url, strategy);
    const durationMs = Date.now() - start;
    if (!result) {
      return {
        url: entry.url, category: entry.category, strategy, success: false,
        priceValid: false, nameValid: false, optionCount: 0, notFound: false,
        error: "null result", durationMs,
      };
    }
    if (result.error === "product_not_found") {
      return {
        url: entry.url, category: entry.category, strategy, success: false,
        priceValid: false, nameValid: false, optionCount: 0, notFound: true,
        error: "product not found", durationMs,
      };
    }
    const priceValid = isValidPrice(result.price);
    const nameValid = Boolean(result.name && result.name.trim().length >= 3);
    return {
      url: entry.url, category: entry.category, strategy, success: priceValid && nameValid,
      name: result.name, price: result.price, method: result.method,
      priceValid, nameValid, optionCount: result.options.length,
      options: result.options.map((o) => `${o.name}: [${o.values.join(", ")}]`),
      notFound: false,
      error: !priceValid ? `invalid price: "${result.price}"` : !nameValid ? "invalid name" : undefined,
      durationMs,
    };
  } catch (err: any) {
    return {
...
async function main() {
  console.log(`\n=== Strategy-Aware Bulk URL Test ===`);
  console.log(`URLs: ${urls.length} | Concurrency: ${CONCURRENCY}`);
  console.log(`FIRECRAWL_BASE_URL: ${process.env.FIRECRAWL_BASE_URL}`);
  console.log(`EXA_API_KEY: ${process.env.EXA_API_KEY ? "set" : "NOT SET"}\n`);

  // Show strategy distribution
  const strategyDist: Record<string, number> = {};
  for (const u of urls) {
    const s = classifyUrl(u.url);
    strategyDist[s] = (strategyDist[s] ?? 0) + 1;
  }
  console.log(`Strategy distribution: ${Object.entries(strategyDist).map(([k, v]) => `${k}=${v}`).join(", ")}\n`);
  ...
  while (queue.length > 0) {
    ...
    for (const r of batchResults) {
      ...
      const errStr = r.error ? ` (${r.error.slice(0, 50)})` : "";
      console.log(
        `[${String(results.length).padStart(2)}/${urls.length}] ${status.padEnd(4)} ${r.strategy.padEnd(12)} ${(r.durationMs / 1000).toFixed(1).padStart(5)}s  ${r.category.padEnd(28)}${r.method ?? ""} ${nameStr}${priceStr}${optStr}${errStr}`,
      );
    }
  }
  ...
  const percentile = (p: number): number => {
    const idx = Math.min(sortedDurations.length - 1, Math.max(0, Math.ceil((p / 100) * sortedDurations.length) - 1));
  ...
  const strategyCounts = results.reduce<Record<string, { pass: number; fail: number; avgMs: number; total: number }>>((acc, r) => {
    if (!acc[r.strategy]) acc[r.strategy] = { pass: 0, fail: 0, avgMs: 0, total: 0 };
    acc[r.strategy].total++;
    if (r.success) acc[r.strategy].pass++; else acc[r.strategy].fail++;
    acc[r.strategy].avgMs += r.durationMs;
    return acc;
  }, {});
  ...
  console.log(`Failed:       ${failed.length} (${notFoundResults.length} not found)`);
  console.log(`W/ Options:   ${withOptions.length}`);
  console.log(`Avg Time:     ${(totalMs / results.length / 1000).toFixed(1)}s`);
  console.log(`P50 Time:     ${(percentile(50) / 1000).toFixed(1)}s`);
  console.log(`P95 Time:     ${(percentile(95) / 1000).toFixed(1)}s`);
  console.log(`Fastest:      ${(Math.min(...results.map((r) => r.durationMs)) / 1000).toFixed(1)}s`);
  console.log(`Slowest:      ${(Math.max(...results.map((r) => r.durationMs)) / 1000).toFixed(1)}s`);
  console.log(`Total Time:   ${(totalMs / 1000).toFixed(0)}s`);
  console.log(`Methods:      ${Object.entries(methodCounts).map(([k, v]) => `${k}=${v}`).join(", ")}`);

  console.log(`\n=== Per-Strategy Breakdown ===`);
  for (const [strat, data] of Object.entries(strategyCounts)) {
    const avgMs = data.avgMs / data.total;
    console.log(`  ${strat.padEnd(14)} ${data.pass}/${data.total} pass (${((data.pass / data.total) * 100).toFixed(0)}%)  avg ${(avgMs / 1000).toFixed(1)}s`);
  }

  if (failed.filter((r) => !r.notFound).length > 0) {
    console.log(`\n--- Failed URLs ---`);
    for (const f of failed.filter((r) => !r.notFound)) {
      console.log(`  [${f.strategy}] ${(f.durationMs / 1000).toFixed(1).padStart(5)}s  ${f.category}: ${f.error?.slice(0, 60)}`);
    }
  }
}
